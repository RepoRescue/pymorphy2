...
def _iter_entry_points(*args, **kwargs):
    """ Like importlib.metadata.entry_points, but uses a fresh discovery.
    This ensures that all entry points are picked up, even if a package
    which provides them is installed after the current process is started.
